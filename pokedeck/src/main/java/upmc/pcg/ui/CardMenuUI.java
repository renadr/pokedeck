...
public class CardMenuUI {
    ...
    public static Card cardConsultMenu(Deck collectionGiven, boolean onlyPokemonCard) {
        ...
        while(!boolIndexOk || collectionGiven.getSize()>1) {
            try {
            ...
            if(chosenIndex>=0 && chosenIndex < collection.getSize()) {
                if(!onlyPokemonCard)
                    boolIndexOk = true;
                else {
        ...
        if(collectionGiven.getSize()<1)
            return null;
        else
            return collectionGiven.getCard(chosenIndex);
    }
    ...
    private static boolean checkIfPokemonCard(Deck collectionGiven, int chosenIndex) {
        ...
        if(collectionGiven.getCard(chosenIndex).getCardType().equals("pokemon"));
        isPokemonCard = true;
     ...
     */
    public static HashMap<String, Object> askPokemonCardAttributes() {
